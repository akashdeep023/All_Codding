...
export default function TodoList() {
  ...
  let deleteAllTask = () => {
    setTodos((prevTodos) => todos.filter((prevTodos) => prevTodos.id == ""));
  };
  let upperCaseAll = () => {
    setTodos((prevTodos) =>
      prevTodos.map((todo) => {
        ...
        {todos.map((todo) => (
          <li key={todo.id}>
            ...
            {todo.task === todo.task.toUpperCase() ? (
              <button onClick={() => upperCaseOne(todo.id)}>LowerCase</button>
            ) : (
              <button onClick={() => upperCaseOne(todo.id)}>UpperCase</button>
            )}
            &nbsp;
            {todo.isDone ? (
              <button onClick={() => completeTask(todo.id)}>UnComplete</button>
            ) : (
